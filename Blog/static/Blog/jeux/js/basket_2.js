document.addEventListener('DOMContentLoaded', async function () {
    ...
    let nb_paniers = 0;
    let MAX_TIME = 20; // Temps en secondes
    ...
    let BALL_X = 8/10 * window.innerWidth;
    let BALL_Y = window.innerHeight - BALL_RADIUS;
    ...
    // Arrow
    let arrow;
    let arrow_additional_length = 0;
    let base_arrow;
    let ARROW_WIDTH = 10;
    let ARROW_BASE_RED = 124;
    let ARROW_BASE_GREEN = 54;
    let ARROW_BASE_BLUE = 65;
    let arrow_additional_red = 0;
    let ARROW_BASE_ANGLE = 2 * Math.PI/3;
    let ARROW_BASE_LENGTH = 50;
    let ARROW_BASE_X = BALL_X - BALL_RADIUS;
    let ARROW_BASE_Y = BALL_Y - BALL_RADIUS/2;
    let ARROW_MAX_LENGTH = 3*ARROW_BASE_LENGTH;
    ...
    class Ball {
      ...
      launch(arrow) { // Lancer la balle à partir de la arrow actuelle
        let force = arrow.length - ARROW_BASE_LENGTH;
        let angle = arrow.angle;

        console?.log('force, angle : ', force, angle)

        ball.dx = Math.cos(angle) * force * LAUNCH_FORCE;
        ball.dy = Math.sin(angle) * force * LAUNCH_FORCE;
        console.log('launched ! : ', ball.dx, ball.dy);
      }
    ...
    class Arrow {
        ...
        constructor(x, y, length, width, angle) {
            ...
            // vecteurs
            this.d_teta = [Math.cos(angle), Math.sin(angle)];
            this.d_norm = [Math.cos(angle + Math.PI/2), Math.sin(angle + Math.PI/2)];

            this.dx = Math.cos(angle); // direction de tir
            this.dy = Math.sin(angle);

            this.sx = -this.dy; // perpendiculaire gauche/droite
            this.sy =  this.dx;
       ...
       draw() {
            ...
            ctx.closePath();
            ctx.fillStyle = `rgb(${ARROW_BASE_RED + arrow_additional_red}, ${ARROW_BASE_GREEN}, ${ARROW_BASE_BLUE})`;
            ctx.fill();
        }
...
document.addEventListener('mouseup', function() {
    if (mouse_state === 'down') {
        try {
            if (ball.x === BALL_X && ball.y === BALL_Y) {
                ball.launch(arrow);
            }
        }
        catch {
            console.log('pas encore de balle');
        }
    }
    mouse_state = 'up';
})
    ...
    function debut_jeu() {
        ...
        arrow = new Arrow(ARROW_BASE_X, ARROW_BASE_Y, ARROW_BASE_LENGTH, ARROW_WIDTH, ARROW_BASE_ANGLE);
        ...
        let timer = setInterval(() => {
            ...
            // Get arrow angle
            let current_angle = Math.atan2(mouse_y - ARROW_BASE_Y, mouse_x - ARROW_BASE_X);
            arrow = new Arrow(ARROW_BASE_X, ARROW_BASE_Y, ARROW_BASE_LENGTH + arrow_additional_length, ARROW_WIDTH, current_angle);
            if (mouse_state === 'down' && ball.x === BALL_X && ball.y === BALL_Y) {

                arrow_additional_length += 1;
                arrow_additional_red += 1;
                if (arrow_additional_length >= ARROW_MAX_LENGTH) {
                    arrow_additional_length = 0;
                    arrow_additional_red = 0;
                }
            }
            else {
                arrow_additional_length = 0;
